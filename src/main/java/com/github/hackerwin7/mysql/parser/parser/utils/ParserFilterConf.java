 ...
 */
public class ParserFilterConf {
    private Logger logger = LoggerFactory.getLogger(ParserFilterConf.class);
    ...
    public static Map<String, String> disSense = new HashMap<String, String>();
    //avro
    ...
    public void initConfOnlineJSON() throws Exception {
        clear();
        //external config
        ConfigJson cj = new ConfigJson(jobId + "_sense", "magpie.address");
        JSONObject jo = cj.getJson();
        if(jo != null) {
            if(jo.containsKey("info")) {
                JSONObject jd0 = jo.getJSONObject("info");
                if(jd0.containsKey("content")) {
                    JSONObject jd = jd0.getJSONObject("content");
                    if (jd.containsKey("db_tab_meta")) {
                        ...
                        for (int i = 0; i <= jf.size() - 1; i++) {
                            ...
                            String mapkey = dbname + "." + tablename;
                            if (jdata.containsKey("sensefield")) {
                                String senseField = jdata.getString("sensefield");
                                disSense.put(mapkey, senseField);
                            }
                        }
                    }
                }
